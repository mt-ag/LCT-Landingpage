...
The way the backend communication works is that our test definition is stored in an Oracle database which runs the LCT instance and then via REST the definition is sent to the nodejs backend server which then executes the test.  The result is then sent back to the DB via REST.
...
- Currently a bachelor thesis is being written about test coverage with LCT. The results will find their way into LCT.
