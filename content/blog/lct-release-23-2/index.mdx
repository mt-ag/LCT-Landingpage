...
Last release was version [0.18](https://lct.software/blog/lct-release-0-18). LCT has been stable quite som while now so the zero in front of the version number didn't really mean that it is in a beta-like state. We finally change that and directly jump to 23 to match Oracles versioning scheme. 23.2 means that this is the second release in 2023. It does not mean that it is only compatible with APEX 23.x or anything like that.

The timing to change the versioning system is also quite nice as this is **our biggest** release ever. I think you will like most changes we made.

## New Features

### Interactive Grid support

You can imagine that this is a not only a highly requested feature but something we worked on for quite some time. But we believe that we got Interactive Grid testing right.
